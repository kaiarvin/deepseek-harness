...
因此只要 `model.reasoning` 为假，`reasoningInfo` 就省略 Service Definition 的 `reasoning` 字段。判据是模型自身的元数据，而非模型的来源。手工声明模型现在默认获得 off / high / max 三档而非没有元数据（[[2026-08-18-hand-declared-default-thinking-offer]] 把该默认给了 catalog 未描述的模型），因此「能力未报告」的情况覆盖的是 pi-ai 标记为不具备推理能力的那 251 个已安装 catalog 模型。它们此前提供那个孤零零的 `off`，现在什么也不提供，界面只剩提供方默认。携带推理元数据的模型不受影响——其档位列表仍不经筛选地穿过 seam、`off` 也在内，因为在那里它是在真实备选之间做选择。
