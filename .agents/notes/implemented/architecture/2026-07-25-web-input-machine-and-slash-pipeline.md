...
- Each reference occupies a run of `U+FFFC` cells in the draft — one 4em pill cell per cell, the run length sized to the label so the pill never clips it; a table entry is `{occurrenceId, source, ref, offset, length, label, clipboardText, invalid?}`; same-named chips stay independent through occurrenceId. The cell count is a text-width estimate at insert time; after paint the composer measures the rendered pill at its natural (max-content) width and resizes the run through the machine's internal `resize-chip` event (one undo unit) to the exact fit — BOTH ways, so an over-estimate shrinks too (a constrained scrollWidth can only report the clamp once the label fits, which cannot detect an over-size).
- Every edit updates the draft and the table in one transaction: ranges shift; a deletion/replacement intersecting a placeholder run acts on the whole chip.
- The placeholder run makes keyboard atomicity mostly hold natively (the caret has no interior position; Backspace / arrow keys / Shift extension natively take the whole chip). The visible pill hugs the label (an inner overlay, not the full cell span), so the run length shows only as spacing before the next glyph.
- The visual projection = label: the backdrop renders the chip at the placeholder offset (the textarea glyph is invisible), with invalid taking the invalid style. A dropped-file chip (absolute-path ref) is a real click target: a transparent click-catcher layer replicates the draft flow ABOVE the textarea, one button per path chip, and clicking opens the file through the same Host opener the chat view's file links use; non-path chips and absent openers keep the textarea caret behavior.
