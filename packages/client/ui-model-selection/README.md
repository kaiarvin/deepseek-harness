...
Model selection plugin, browser half: TWO entries over ONE per-session directory owned by `ModelDirectoryResolver` (`ctx.modelDirectories`). For ordinary sessions, the `/model` popupSelect contribution (registered through `ctx.commandUi`) and the composer's named `conversation.input.model` seat both load the session's advisory directory through `session.models` and submit through `session.selectModel` via the same `ModelDirectory` instance. The compact composer trigger opens straight into the provider-grouped model list; beside it the seat renders a quick thinking-level dropdown whenever the current model offers any of the three DeepSeek levels: the fixed localized choices 关闭 / 高 / 最高 map to `off` / `high` / `max` and submit through the same `session.selectModel`, so the thinking strength is one click away without a menu drill-down, and the model chip itself stays a pure model picker.

The Host-reported provider/model/reasoning `ModelSelection` is the single selection fact, but it is echoed only when the exact provider/model pair remains in the advertised groups; an absent catalog row leaves the routable selection intact while the trigger prompts `Select model`, no stale row is synthesized, and the reasoning level shown by the thinking chip reflects the persisted selection plus the adapter default until the user picks an advertised model. Directory loads and selections share a generation counter so an older response never overwrites a newer one; a connection reset drops every resident projection and repulls the Host-restored selection before display. Provider-local metadata failures list inline while usable groups stay selectable, and selection failures retain the prior selection and directory.
...
- **No arbitrary effort input** — the thinking chip offers only the fixed DeepSeek levels (`off` / `high` / `max`), gated on the exact model advertising them; an adapter without those three levels keeps the chip hidden and no generic effort surface remains in the composer.
