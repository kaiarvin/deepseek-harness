...
模型选择插件（浏览器侧）：**两个入口共用一份会话级目录**，由 `ModelDirectoryResolver`（`ctx.modelDirectories`）持有。对于普通会话，`/model` popupSelect 贡献项（经 `ctx.commandUi` 注册）与 composer 的具名 `conversation.input.model` slot 都通过同一个 `ModelDirectory` 实例，经 `session.models` 加载会话的建议目录，并经 `session.selectModel` 提交。紧凑型 composer 触发器直接展开按提供方分组的模型列表；其旁边，只要当前模型提供 DeepSeek 三档思考级别中的任意一档，slot 就渲染一个快捷思考级别下拉：固定的本地化选项 关闭 / 高 / 最高 对应 `off` / `high` / `max`，并经由同一个 `session.selectModel` 提交，一次点击即可调整思考强度，无需钻入菜单；模型芯片本身只负责选模型。

Host 报告的 `ModelSelection` 是唯一的选择事实，其中包含提供方、模型与推理（reasoning）强度；但只有当该提供方／模型对仍在已公布分组中时才会回显。目录行缺席时，可路由的选择保持不变，但触发器会提示 `Select model`；系统不会合成陈旧行，思考芯片显示的推理等级反映持久化选择叠加适配器默认值，直到用户选了一个已公布的模型。目录加载与选择共享一个代次计数器，旧响应不会覆盖新结果；连接重置会丢弃所有常驻目录投影，并在显示前重新拉取 Host 恢复的选择。各提供方的元数据获取失败会内联列出，同时可用分组仍可选择；选择失败会保留先前的选择和目录。
...
- **不能任意输入推理强度**——思考芯片仅提供固定 DeepSeek 档位（`off` / `high` / `max`），且只在确切模型公布这三档时显示；不提供三档的模型保持芯片隐藏，composer 中不再保留通用的推理强度面板。
