...
import { usageReportValueSchema } from '../api/usage.schema.ts'
import {
...
export interface IApiClient {
  ...
  usage: {
    report(payload: RequestPayload<'usage.report'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'usage.report'>>>
  }
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'usage.report': usageReportValueSchema,
}
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly usage: IApiClient['usage'] = {
    report: (payload, signal) => this.callUnary('usage.report', payload, signal),
  }
